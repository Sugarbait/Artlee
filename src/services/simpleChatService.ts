  ...
  async sendMessage(userMessage: string): Promise<SimpleChatResponse> {
    try {
      ...
      const messages: SimpleChatMessage[] = [
        {
          role: 'system',
          content: 'You are a helpful assistant for the ARTLEE CRM platform. Help users navigate and use platform features. Use natural language with elegant, well-structured sentences and numbered lists when appropriate. Do NOT use markdown formatting. Write in a conversational, professional tone.'
        },
        {
          role: 'user',
          content: userMessage
